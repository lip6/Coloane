 ...
 */
public final class MultiChoicesDialogConstructor
    implements ItemDialog<MultipleChoiceParameter> {
    ...
    public MultiChoicesDialogConstructor(final ParametersPage page, final Composite parent,
                                         final MultipleChoiceParameter parameter) {
    ...
    public
        ...
        MultipleChoiceParameter that = MultipleChoiceParameter.of(parameter);
        if (this.parameter.equals(that)) {
            list.setBackground(null);
        } else {
            for (String choice : that.getChoices()) {
                if (that.getValues()
                        .contains(choice)) {
                    list.select(Math.max(0, that.getChoices()
                                                .indexOf(choice)));
                } else {
                    list.deselect(Math.max(0, that.getChoices()
                                                  .indexOf(choice)));
                }
            }
            list.setBackground(list.getDisplay()
    ...
    @Override
    public
        MultipleChoiceParameter getParameter() {
        return parameter;
    }
