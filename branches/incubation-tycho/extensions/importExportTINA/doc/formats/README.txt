These files describe the formats of files used by the Tina tools:

Time Petri nets formats, input format of tina/struct/plan/nd:
...
Time Transition System format, input format of tina/struct/plan
(not yet supported by nd):

tts.txt  	.tts 	Time Transition System format

Transition system formats, output formats of tina:
...
ltl.txt  	.ltl 	Selt SE-LTL model checker commands (selt)
scn.txt		.scn 	Format of firing sequences and firing schedules,
			(selt, plan, nd stepper history files)
