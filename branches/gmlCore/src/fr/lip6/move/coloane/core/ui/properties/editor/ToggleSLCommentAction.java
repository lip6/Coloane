...
public final class ToggleSLCommentAction extends Action implements IUpdate {
	...
	private IRegion getTextBlockFromSelection(ITextSelection selection, IDocument document) {
		...
		try {
			...
			if (selection.getLength() == 0) {
				...
				length =  selection.getLength() + (selection.getOffset() - line.getOffset());
			}
	...
	/**
	 * Configure the comment action.
	 * @param sourceViewer The viewer associated to the action.
	 * @param configuration The configuration class for the viewer.
	 */
