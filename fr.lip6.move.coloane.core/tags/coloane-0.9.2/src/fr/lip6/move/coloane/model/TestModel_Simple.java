...
public class TestModel_Simple extends TestCase {
	...
	/*
	 * Ajout d'un arc dans le model cas 1 - l'arc n'a ni cible ni cas 2 - l'arc
	 * a une source mais pas de cible cas 3 - l'arc a une cible et une source
	 */
	...
	/*
	 * Test si un arc relié à 2 noeuds non présent dans le modèle peut être
	 * ajoutté
	 */
	...
	public void testRemoveNode() {
		...
		for (int i = 0; i < listOfInArc.size(); i++) {
			...
			/*
			 * arc entrant plus présent dans la liste des noeuds du modele et de
			 * sa cible
			 */
			assertTrue(model.getAnArc(id_arc) == null);
		...
		for (int i = 0; i < listOfOutArc.size(); i++) {
			...
			/*
			 * arc sortant plus present dans la liste des noeuds du modele et de
			 * sa source
			 */
			assertTrue(model.getAnArc(id_arc) == null);
