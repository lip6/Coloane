		...
		while (tour<=max_tour){
			...
			if(tour%8==0 && !(model.getListOfArcSize()==0)){
				...
				while(i<nb_remove){
					...
					arc_remove=model.getNthArc(ind);
					if(!listOfArcToRemove.contains(arc_remove)){
					listOfArcToRemove.addElement(arc_remove);
					i++;
					System.out.println("Ajout de l'arc "+model.getNthArc(ind).getId()+" à listOfArcToRemove tour:"+tour+"\n");
					}
				}
				...
				for (int j=0;j<listOfArcToRemove.size();j++){
					...
					if(action_alea==0){
						...
						else{System.out.println("ListOfNode vide - tour:"+tour+"\n");}
					}
					else{
