			...
			for (int k = 0; k < camiCommande.size(); k++) {
				...
				if (type.equals("CN")) {
					...
					if (Integer.parseInt(nodeId) != 1) {
						INode node = new Node(nodeType, 0, 0, Integer
								.parseInt(nodeId));
						this.addNode(node);
					} else {
				...
				if (type.equals("CA")) {
					...
					if (nodeBegin == null || nodeEnd == null) {
						throw new SyntaxErrorException(
								"Noeud source ou cible manquant");
					}
				...
				if (type.equals("CT")) {
					...
					IAttribute attr = new Attribute(name, value, Integer
							.parseInt(ref));
				...
				if (type.equals("CM")) {
					...
					try {
						...
						throw new SyntaxErrorException(
								"Construction d'un attribut multiligne incorrecte");
					}
				...
				if (type.equals("PO") || type.equals("pO")) {
					...
					if (Integer.parseInt(ref) == -1) {
						...
						y = ps.parseInt(",");
						// x = ps.parseInt(",");
						System.out.println("Ref : " + ref + " x=" + x + " y="
								+ y);
					} else {
						x = ps.parseInt(",");
						y = ps.parseInt(")");
					...
					if (Integer.parseInt(ref) == 1) {
						this.setPosition(Integer.parseInt(x), Integer
								.parseInt(y));
					}
					...
					if (Integer.parseInt(ref) != 1) {
						INode node = getANode(Integer.parseInt(ref));
						if (node != null) {
							node.setPosition(Integer.parseInt(x), Integer
									.parseInt(y));
						} else {
							throw new SyntaxErrorException(
									"La position est attachee a un element introuvable ou incorrect");
						}
					}
				}
				...
				if (type.equals("PT")) {
					...
					if (Integer.parseInt(ref) == 1) {
						for (int i = 0; i < this.getListOfAttrSize(); i++) {
							...
							if (attr.getName().equals(name)) {
								attr.setPosition(Integer.parseInt(x), Integer
										.parseInt(y));
								break;
							}
						}
					...
					throw new SyntaxErrorException(
							"Impossible d'attachee la position de texte a un attribut");
				}
			}
